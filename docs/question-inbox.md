...
| Prose-stop nudge [`nudge-prompt.ts`](../packages/server/src/server/agent/prose-stop/nudge-prompt.ts)            | Turn-end backstop: pushes agents away from prose and toward ask_question                                         |
| Prevention prompt [`prevention-prompt.ts`](../packages/server/src/server/agent/prose-stop/prevention-prompt.ts) | `proseStop.preventionPrompt` (default on): short `daemonAppendSystemPrompt` so agents avoid prose waits up front |
