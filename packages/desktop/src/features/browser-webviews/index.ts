...
// Host-agnostic lookup kept for the fork's inline DevTools and cookie import:
// first live registration for the browser id across host windows.
export function getPaseoBrowserWebContents(browserId: string): WebContents | null {
  const contentsId = browserRegistry.getAnyWebContentsIdForBrowser(browserId);
  if (contentsId === null) {
    return null;
  }
