      ...
      items: {
        ...
        properties: {
          file: {
            type: "string",
            description:
              "repo-relative path exactly as listed under Changed files in the review scope",
          },
          line: { type: "number" },
          summary: { type: "string" },
    ...
    verdicts: {
      ...
      items: {
        ...
        properties: {
          index: {
            type: "number",
            description: "the [i] label of the candidate this verdict is for",
          },
    ...
    decisions: {
      ...
      items: {
        ...
        properties: {
          index: {
            type: "number",
            description: "the [i] label of a finding to keep in the report",
          },
          merge: {
            type: "array",
            items: { type: "number" },
            description:
              "[i] labels of findings that describe the same root cause, folded into this one",
          },
        },
      },
    },
  ...
  const out = await parallel(
    groups.map((g) => async () => {
      ...
      return g.flatMap((c, i) =>
        byIdx[i] ? [{ ...c, verdict: byIdx[i].verdict, evidence: byIdx[i].evidence }] : [],
      );
    }),
  );
...
const finderOuts = await parallel(
  FINDERS.map(
    (f) => () =>
      agent(FINDER_PROMPT(f), { label: f.label, phase: "Find", schema: CANDIDATES_SCHEMA }).then(
        (r) => {
          if (!r) return [];
...
for (const d of decisions) {
  ...
  const also =
    merged.length > 0 ? " [same root cause also at: " + merged.map(loc).join(", ") + "]" : "";
  findings.push({
    file: c.file,
    line: c.line,
