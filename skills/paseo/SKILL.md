...
**`dispatch_workflow`** — `{ definition, cwd?, task?, provider?, model?, args?, resumeFromRunId? }`. `provider`/`model` fold into args as the run's agent() defaults — set them when the daemon's default provider may be disabled. `definition` takes a definition id or a `*.flow.js` path (absolute or relative to cwd) — paths dispatch read-through. Flows that manage their own worktrees (ticket pipelines) want `cwd` = the MAIN repo root; don't pre-create a worktree for them. After a failed run, prefer `resumeFromRunId` over re-dispatching: successful stages replay from the prior run's journal at zero cost.
