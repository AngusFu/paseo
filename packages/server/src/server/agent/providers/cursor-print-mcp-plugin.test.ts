...
describe("cursor-print-mcp-plugin", () => {
  ...
  test("materializeCursorPrintMcpPlugin writes MCP-only plugin", () => {
    ...
    expect(existsSync(join(plugin.pluginDir, "rules"))).toBe(false);
    expect(existsSync(join(plugin.pluginDir, "hooks"))).toBe(false);
