...
/**
 * Fetch inbox questions across connected hosts and merge them into one flat list.
 * Offline hosts are skipped. Connected hosts that fail contribute to `hostErrors`.
 */
export async function fetchAggregatedQuestions(
  ...
  if (connectedAttempts > 0 && questions.length === 0 && hostErrors.length === connectedAttempts) {
    throw new Error(ALL_QUESTION_HOSTS_FAILED_MESSAGE);
  }

  if (questions.length === 0 && hasSettlingHost) {
    return { status: "connecting" };
  }

  questions.sort((left, right) => left.createdAt.localeCompare(right.createdAt));
  return { status: "loaded", data: questions, hostErrors };
}
